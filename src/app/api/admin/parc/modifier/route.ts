...
export async function POST(req: Request) {
  ...
  if (!actuelle) return versPage(req, "/admin");
  ...
  if (!statutValide(statutDemande)) return versPage(req, "/admin?e=statut");
  ...
  if (!actuelle.jeton) compte_id = compteBrut ? Number(compteBrut) : null;
    ...
    return versPage(req, `/admin?e=appairee#m${id}`);
  }
  ...
  try {
    ...
    if ((e as { code?: string }).code === "23505") return versPage(req, `/admin?e=numero#m${id}`);
    throw e;
  }
